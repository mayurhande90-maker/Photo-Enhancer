...
async function processImageWithAI(
  ...
  if (userId && result.enhancedPhotoDataUri) {
    await saveGeneratedImage(
        userId,
        photoDataUri, // The original image
...
export async function enhancePhotoAction(photoDataUri: string, userId: string) {
  ...
  const result = await processImageWithAI(photoDataUri, prompt, userId, 'Enhancement');
  return result;
}
...
export async function removeBackgroundAction(photoDataUri: string, userId: string) {
  ...
  const result = await processImageWithAI(photoDataUri, prompt, userId, 'Background Removal');
  return result;
}
...
export async function studioEnhanceAction(photoDataUri: string, userId: string) {
  ...
  const result = await processImageWithAI(photoDataUri, prompt, userId, 'Studio');
  return result;
}
...
export async function colorizePhotoAction(photoDataUri: string, userId: string) {
  const prompt =
    ...
    const result = await processImageWithAI(photoDataUri, prompt, userId, 'Colorize');
    return result;
}
